...
class RelanceDesVisasTests(BaseDocuments, TestCase):
    ...
    def test_document_dormant_relance_le_role_attendu(self):
        ...
        # L'administrateur reçoit aussi une notification « Suivi — … » sur
        # chaque relance, pour le suivi global (cf. _notifier_admins_suivi) :
        # le rôle attendu à viser (comptable) N'EST PAS seul destinataire.
        destinataires = set(
            Notification.objects.values_list("utilisateur_id", flat=True))
        self.assertEqual(destinataires, {self.comptable.pk, self.admin.pk})
