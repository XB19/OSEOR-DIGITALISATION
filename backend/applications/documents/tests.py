...
class CreationTests(BaseDocuments, APITestCase):
    ...
    def test_sans_chaine_de_visas_le_document_est_valide_d_office(self):
        # Une chaîne de visas explicitement vide (type configuré, mais sans
        # étape) auto-valide — à distinguer d'une filiale/type qui n'a
        # AUCUNE ConfigurationDocument, qui est refusé (cf.
        # test_type_non_configure_refuse) pour ne jamais auto-valider par
        # défaut faute de configuration.
        ConfigurationDocument.objects.create(
            filiale=self.oseor, type_document=TypeDocument.FICHE_BESOIN,
            colonnes=[], visas=[],
        )
    ...
    def test_type_non_configure_refuse(self):
        """
        Aucune ConfigurationDocument pour cette filiale/type : la soumission
        est refusée plutôt que silencieusement auto-validée sans circuit de
        validation (risque corrigé — une nouvelle filiale ne doit jamais
        valider ses documents faute de configuration).
        """
        self.client.force_authenticate(self.employe_oseor)
        reponse = self.client.post("/api/documents/", {
            "type_document": TypeDocument.FICHE_BESOIN,
